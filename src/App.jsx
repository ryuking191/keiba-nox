...
✅ 【絶対遵守ルール】
1. 独自の挨拶や前置きは【一切禁止】です。いきなり【1. コース概要】から出力してください。
2. 資金配分は【必ず100円単位】です。
3. あなたが「10点」を選択した場合は合計金額が【ぴったり ${budget10} 円】、「16点」を選択した場合は合計金額が【ぴったり ${budget16} 円】になるように割り振ってください。
4. 【超重要】買い目（資金配分）を出力する際は、馬名ではなく【必ず馬番のみ】をハイフン（-）や矢印（→）で繋いで表記してください。（例：馬連（1-2）、三連単（1→2→3）など）
5. 期待値について、◎は「勝率期待値」、〇・▲・△は「連対期待値」として算出し、記載してください。
...
・馬単（◎の馬番→〇の馬番）：〇〇00円
・ワイド（◎の馬番-〇の馬番）：〇〇00円
（※以下、選択したパターンの買い目を【馬番の数字のみ】でリスト化。合計金額を必ず一致させること）

【4. オッズと期待値】
・◎：【〇番】（馬名） 単勝オッズ〇〇倍 / 勝率期待値〇〇％
・〇：【〇番】（馬名） 単勝オッズ〇〇倍 / 連対期待値〇〇％
・▲：【〇番】（馬名） 単勝オッズ〇〇倍 / 連対期待値〇〇％
・△：【〇番】（馬名） 単勝オッズ〇〇倍 / 連対期待値〇〇％
（※なぜその印になったか、オッズの歪みについての解説）
