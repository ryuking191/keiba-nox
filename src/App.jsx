...
✅ 【絶対遵守ルール：資金配分】
1. AIは独自の金額傾斜（強弱）をつけないでください。足し算でエラーを起こすためです。
2. すべての買い目の購入金額は、ユーザーが指定した「1点あたりの金額（＝ ${unitPrice}円 ）」で【完全固定】してください。
3. 50円や150円などの異なる金額は【絶対に使用禁止】です。必ず全ての買い目を「${unitPrice}円」として出力してください。

✅ 【絶対遵守ルール：期待値計算】
1. 「期待値」とは「予想確率（％）× オッズ」で求められる【回収率】のことです。
2. 例：AIの予想勝率が25%で、オッズが5.0倍の場合、期待値は「25 × 5.0 ＝ 125%」となります。100%を超える数値がオッズの歪み（妙味）です。
3. あなたの出力する「期待値30%」などは「予想確率」と「期待値（回収率）」を混同しています。必ず掛け算を行って100%前後の数値（期待回収率）を出力してください。
...
【3. 資金配分】
・合計予算：〇〇円（10点なら${budget10}円、16点なら${budget16}円）
・単勝（〇）：${unitPrice}円
・馬連（〇-〇）：${unitPrice}円
・馬単（〇→〇）：${unitPrice}円
・ワイド（〇-〇）：${unitPrice}円
（※以下、選択したパターンに従って買い目をリスト化し、金額はすべて必ず「${unitPrice}円」と記載すること）

【4. オッズと期待値】
・◎：【〇番】〇〇〇 単勝〇.〇倍 / 予想勝率〇〇％ / 勝率期待値〇〇％
・〇：【〇番】〇〇〇 単勝〇.〇倍 / 予想連対率〇〇％ / 連対期待値〇〇％
・▲：【〇番】〇〇〇 単勝〇.〇倍 / 予想連対率〇〇％ / 連対期待値〇〇％
・△：【〇番】〇〇〇 単勝〇.〇倍 / 予想連対率〇〇％ / 連対期待値〇〇％
※解説：（オッズの歪みなど）
...
※AIへの最終絶対指示：
いかなる理由があっても、指定された【出力フォーマット】以外の文字から書き始めないでください。資金配分の金額はすべて「${unitPrice}円」で完全に統一し、期待値は必ず「確率(%) × オッズ」の掛け算で算出した回収率(%)を提示してください。
      `;日の出馬表・オッズデータ】
${input}
